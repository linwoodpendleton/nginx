...
"
  ...
  make -j"$(nproc)"
  make install
  tar -zxvf conf.tar.gz -C /usr/local/nginx/
...
http
    {
        include       mime.types;
    lua_package_path "/usr/local/share/lua/5.1/?.lua;/usr/local/lib/lua/?.lua;;";
...
server
    {
        ...
        #listen [::]:29801 default_server ipv6only=on;
        server_name _;
        ...
        include enable-php8.3-pathinfo.conf;
